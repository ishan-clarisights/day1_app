...
class FacebookAdsController < ApplicationController
  ...
  private
    ...
    def fetch_and_store_account_insights_data(account_id, date, db)
      ...
      if !response.nil? && !JSON.parse(response).nil? && !JSON.parse(response)['data'].nil? && !JSON.parse(response)['data'].empty?
        ...
        all_insights = {
          ...
          ctr: insights_data[0]["ctr"].nil? ? 0 : insights_data[0]["ctr"].to_f,
          inline_link_click_ctr: insights_data[0]["inline_link_click_ctr"].nil? ? 0 : insights_data[0]["inline_link_click_ctr"].to_f,
          clicks: insights_data[0]["clicks"].nil? ? 0 : insights_data[0]["clicks"].to_i,
          inline_link_clicks: insights_data[0]["inline_link_clicks"].nil? ? 0 : insights_data[0]["inline_link_clicks"].to_i,
          cost_per_inline_link_click: insights_data[0]["cost_per_inline_link_click"].nil? ? 0 : insights_data[0]["cost_per_inline_link_click"].to_f,
          impressions: insights_data[0]["impressions"].nil? ? 0 : insights_data[0]["impressions"].to_i,
          spend: insights_data[0]["spend"].nil? ? 0 : insights_data[0]["spend"].to_f,
